...
class UploadPhotoMixin:
    ...
    def photo_rupload(
            self, image, upload_id: str = "", to_album: bool = False
        ) -> tuple:
        """
    ...
    def photo_upload(
        ...
        upload_id, width, height = self.photo_rupload(image, upload_id)
        for attempt in range(10):
            self.logger.debug(f"Attempt #{attempt} to configure Photo: {image}")
            time.sleep(3)
